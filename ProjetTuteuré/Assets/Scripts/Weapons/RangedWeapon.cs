    ...
    public void Fire(Vector2 mousePosition)
    {
        ...
        if (ammunition != 0)
        {
            ...
            for (int i = 0; i < nbBalles; i++)
            {
                ...
                Vector3 originDirection = Camera.main.ScreenToWorldPoint(new Vector3(
                    mousePosition.x + UnityEngine.Random.Range(-spread/2, spread/2),
                    mousePosition.y + UnityEngine.Random.Range(-spread/2, spread/2),
                    10f));//10.0f car si z = 0f, la fonction retourne la position de la caméra
                ...
                {
                    projectileInstance.GetComponent<Projectile>().damage *= 2f;
